...
function Search(){
    ...
    const sQuery = search ? search.get('s') : null;
    if(sQuery){
        if(isActive){ 
    ...
    if(!isActive){
        ...
        const API_KEY = "22bd8561d5f68d41d4d6f2f5e3e28ab2";
          fetch(`https://api.themoviedb.org/3/search/movie?page=${page}&query=${searchQuery}&api_key=${API_KEY}`, requestOptions)
    ...
    return(
        <div className="container mx-auto min-h-screen my-4">
            <div className="flex justify-center items-center flex-col">
                <h2 className="text-white font-bold">Search : <span className=" text-cyan-300 font-normal border-4 border-solid border-gray-600 rounded-2xl p-2 mx-2">{searchQuery}</span></h2>
